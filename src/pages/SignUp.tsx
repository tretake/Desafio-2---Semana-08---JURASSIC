  ...
  return (
    <div className="flex lg:justify-between justify-center relative lg:h-[calc(100vh-234px)] ">
      ...
      <div className="flex-1 bg-white opacity-95 lg:my-[0] my-[87px] mx-[44px] rounded-[30px]">
        <div className="lg:pt-[58.46px] lg:pl-[100px] sm:px-[44px] sm:py-[30px] px-[20px] py-[20px]">
          ...
          <p className="text-[#331436] sm:text-left text-center text-sm lg:text-base mb-6">
            Already have an account? <a href="/login" className="font-bold text-blue-500 text-center md:text-right hover:underline cursor-pointer">Log in.</a>
          </p>
          {error && <p className="">{error}</p>}
          <form onSubmit={handleSignUp} className="space-y-6">
            <div className="flex sm:flex-row flex-col gap-4">
              <div className="">
                <label htmlFor="firstname" className="text-sm font-medium text-[#331436] mb-1">First name</label>
                <input
                  id="firstname"
                  type="text"
                  value={firstname}
                ...
                />
                {firstnameError && (
                  <span className="block text-red-500 text-sm">{firstnameError}</span>
                )}
              </div>
              <div>
                ...
                />
                {lastnameError && (
                  <span className="block text-red-500 text-sm">{lastnameError}</span>
                )}
              </div>
            </div>
            <div className="">
            ...
            <div className="">
              ...
              />
              {jobpositionError && (
                <span className="block text-red-500 text-sm">{jobpositionError}</span>
              )}
            </div>
            <div className="">
              ...
              />
              {passwordError && (
                <span className="block text-red-500 text-sm mt-2 sm:max-w-lg max-w-xs break-words text-center">{passwordError}</span>
              )}
            </div>
      ...
      <div className="flex-1 pl-5 hidden lg:block">
        <img src={ManInPc} className="top-0 left-0 w-full h-full object-cover"/>
      </div>
    </div>
  );
};
