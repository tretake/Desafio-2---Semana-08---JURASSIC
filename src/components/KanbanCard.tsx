...
const KanbanCard: React.FC<CardProps>  = ({label,priority,color,image}) => {
  ...
  return (
    ...
    p-[4.22px]  w-[97.85px] rounded-[8.45px]
    md:p-[7.94px]  md:w-[184px]  md:rounded-[15.89px]
    lg:p-3 lg:w-[278px]  lg:rounded-3xl}`} >


      { image? <img className=' lg:mb-4 md:mb-[10.59px] mb-[5.63px] lg:w-[246px] lg:rounded-2xl md:w-[162.83px] md:rounded-[10.59px] w-[86.58px]  rounded-[5.63px] ' src={image} alt="" /> : <></> }

      <div>
        <div className={` ${getPriorityColor()} rounded-full text-center font-semibold
        text-[4.22px] w-[13.63px] h-[8.82px]  
        md:text-[7.94px] md:w-[25.59px] md:h-[16.3px] 
        lg:text-sm lg:w-[41px] lg:h-6 `} >{priority}</div>
        
        <h1 className='font-bold 
        text-[5.63px] mt-[4.22px]
        md:text-[10.59px] md:mt-[7.94px]
        lg:text-base lg:mt-3
        ' >{label}</h1>
      </div>
   
      <div className='my-[5.63px] md:my-[10.59px] lg:my-4'>
        <div className='flex justify-between text-[5px] md:text-[9.27px] lg:text-sm'>
          <p className='text-[#475569] ' >Progress</p>  
          <p className='font-bold' > {percentage}%</p>
        </div>
        <div className="w-full bg-[#E2E8F0] rounded-full h-[2.83px] md:h-[5.3px] lg:h-2 ">
            <div className= { ` bg-[${getColor("item")}] h-[2.83px]  md:h-[5.3px] lg:h-2 rounded-full  `}  style={{width: `${percentage}%` }}></div>
        </div>
      </div>
